...
//如果你编译172行报错，请点击工具-编译选项-勾选编译时加入以下命令，在下面的框中输入：-std=c++11，确定即可
#include <iostream>
#include <string>
#include <windows.h>
#include <conio.h>
...
int fight(string hn, int hmh, int ha, int hd, int hh)
{
   while (1)
   {
      if (nowweapon.type == "剑")
         cout << "1,扎刺";
      else if (nowweapon.type == "刀")
         cout << "1,劈砍";
      else if (nowweapon.type == "机枪")
         cout << "1,扫射";
      else if (nowweapon.type == "手枪")
         cout << "1,射击";
      else if (nowweapon.type == "")
         cout << "1,拳击";
      ...
      if (which == "1")
      {
         if (nowweapon.type == "剑" || nowweapon.type == "刀" || nowweapon.type == "")
         {
            ...
            cout << "[" << health << "/" << maxhealth << "]        @E[" << hh << "/" << hmh << "]";
            if (skills[5])
               hh -= (attack + nowweapon.attack) * 2 - hd;
            else
               hh -= attack + nowweapon.attack - hd;
            Sleep(10);
            system("cls");
            cout << "\n\n\n\n\n";
            cout << "[" << health << "/" << maxhealth << "]      @  E[" << hh << "/" << hmh << "]";
...
int forest()
{
   int x = 51, y = 14;
   char c;
   for (int i = 0; i < opxi; i++)
      f[Outposty[i]][Outpostx[i]] = 'P';
   while (1)
   {
      system("cls");
      ...
      water--;
      if (water == 0)
         MessageBox(0, TEXT("水喝完了"), TEXT("重要提示"), MB_OK);
      if (water == -1)
         MessageBox(0, TEXT("你感觉口干舌燥"), TEXT("重要提示"), MB_OK);
      if (water == -2)
      {
      ...
      else if (f[y + movey[move]][x + movex[move]] == 'U')
      {
         ...
         jiguangbuqiang = rand() % 2 + 1;
         shoulai = rand() % 3 + 2;
         daodan = rand() % 3 + 1;
         zidan = rand() % 50 + 50;
         buqiang = rand() % 5 + 1;
         waixinghejin = rand() % 5 + 5;
         tie = rand() % 10 + 10;
         mei = rand() % 10 + 10;
         gang = rand() % 10 + 10;
